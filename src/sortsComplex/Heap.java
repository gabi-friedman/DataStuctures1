...
public class Heap<T extends Comparable<T>> {
    ...
    public void add(T nextVal)throws RuntimeException{
    	...
    	if (sorted) throw new RuntimeException("heap is sorted, can't add value");
    	...
    	//than the value that was just added 
    	while (parent >=0 && heap.elementAt(place).compareTo(heap.elementAt(parent)) > 0){
    		//swap heap[place] with heap[parent] since it isn't in the correct order
    ...
    public T remove()throws RuntimeException{
    	...
    	if (isEmpty()) throw new RuntimeException("heap is empty, nothing to remove");
    	T temp = heap.elementAt(0);  //references the object at top of the heap
    	--size;   //reduces size of the heap
    	heap.set(0,heap.elementAt(size));  //places last value of heap, at "root" of heap
    ...
    private void rebuildHeap(int root){
     ...
     //continue this process if 
    	...
    	if (child <= last){
    		...
    		if ((rChild <= last) && 
    		    heap.elementAt(rChild).compareTo(heap.elementAt(child)) >0)		
    		   			child = rChild;  //index of larger of the children
    		
    		//if the current root's value is smaller than the value in the larger child, 
    		//swap the values
    		if (heap.elementAt(root).compareTo(heap.elementAt(child)) < 0)
    		{  //swap the values
		 ...
		 if (!sorted){
		    //begin sort process
			 ...
			 for (int step =1; step < size; step++){
				 ...
				 //we know that the first value is the largest value in the data set
